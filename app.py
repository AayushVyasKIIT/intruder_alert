...
family = {"add_your_ip_here":"brother","father":"mom"}

while_list = ["add_your_ip_here",IP_LAPTOP]
...
while(True):
    ...
    for host in nm.all_hosts():
        ...
        if host in family.keys():
            notification.update("Family Connect!","Device: <b>"+family[host]+"</b>",
                    "dialog-error")
            notification.show()
        ...
        if host == IP_IPHONE and showed == False:
            notification.update("Your Device connected","<b>iPhone: "+str(host)+"</b>",
                    "dialog-apply")
            notification.show()
            notification.set_urgency(0)
            showed = True
